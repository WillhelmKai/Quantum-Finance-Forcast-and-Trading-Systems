...
# network strcture
ph_input_vector = tf.placeholder(tf.float32,[None, batch_size,4]) #[len, batch, a record]
ph_label_vector = tf.placeholder(tf.float32,[1,3])
ph_latest_OPEN = tf.placeholder(tf.float32,[1,1])

cell = tf.nn.rnn_cell.GRUCell(num_units = 120)
init_state = cell.zero_state(batch_size=1,dtype = tf.float32)#batch size intented to be, out can be [-1, 1100] multiply oneby one
...
W_fc_1 = weight_variable([121, 300])
...
with tf.Session() as sess:
    ...
    #############################
            ...
            for record in testing_set:
                input_vector = np.reshape(record[:length_of_training_records-1], (length_of_training_records-1,batch_size,-1))
            ...
            input_vector = np.reshape(latest_day, (length_of_training_records-1,batch_size,-1))
...
###############
        ...
        for i in range(0,epoch):
            training_rate = 2e-4/(1+np.log(i+1))
            count = 0
            ...
            for record in training_set:
                ...
                # prediction = sess.run(final_state, feed_dict = {
                #     ph_input_vector:input_vector,
                #     ph_latest_OPEN:OPEN,
                #     ph_label_vector:label_vector
                #     })
